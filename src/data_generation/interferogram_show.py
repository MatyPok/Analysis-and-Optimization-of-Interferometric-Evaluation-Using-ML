...
config = load_config(os.path.join(wdir,"src/data_generation/config_0.json"))


#wavelength = 632.8e-9  # Vlnová délka (He-Ne laser) v metrech
wavelength = config["wavelength"]
wavelength = 1
# Velikost snímku (sizexsize pixelů)
...
def Generate(coefficients):
    ...
    return intensity_circular, phase_circular, [coefficients[0], coefficients[1]], coefficients[2:]
...
def main(n):
    ...
    intens, phases, tilt, target_coefficients = map(np.array, zip(*intens_phases_and_such))
    #intens = np.array(intens_phases_and_such)

    end = time()
    print(f"Execution time: {end - start} seconds")
    
    intens = np.transpose(intens, (1,2,0))
    intens = np.reshape(intens, (size,size))
    #intens = 1-intens

    from matplotlib import pyplot as plt

    plt.imshow(intens, cmap="binary_r")
    plt.show()


    """rom PIL import Image
    im = Image.open("data/unet_dataset_3/train/images/1a_zygo_b.bmp")
    p = np.array(im)
    plt.imshow(p, cmap="binary_r")
    plt.show()
    """
